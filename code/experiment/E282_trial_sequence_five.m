...
%FramesBufferedtoc
    ...
    if ~too_early && outOrigen && GetSecs<t0+exp.sound.seqBeeps_time*4+fixt-.05                                                                                               
%         earlymoveFrame = data.FrameNumber; 
        display(sprintf('%4.2f s to fourth beep, TOO EARLY',GetSecs-(t0+exp.sound.seqBeeps_time*4+fixt)))
        too_early    = 1;
    end 
    ...
    if ~too_fast && (onLeft || onRight) && GetSecs<t0+exp.sound.seqBeeps_time*5+fixt-.05                                                                                               
%         earlymoveFrame = data.FrameNumber; 
        display(sprintf('%4.2f s to last beep, TOO FAST',GetSecs-(t0+exp.sound.seqBeeps_time*5+fixt)))
        too_fast    = 1;
    end 
    
    if ~too_slow && ~too_fast && (onLeft || onRight) && treach>t0+exp.sound.seqBeeps_time*5+fixt+result.trial_maxRT(next_trial)                                                                                                      
%         treach-t0
%         GetSecs-t0
        too_slow    = 1;
    end
    
    if (too_early && stim_delivered && ~moved) || (~too_early && outOrigen && stim_delivered &&  ~moved)
    ...
    if ~too_early && ~too_late && ~too_fast && ~too_slow && reached && ~trial_ready
        PsychPortAudio('Stop', pahandle);
        PsychPortAudio('FillBuffer',...                                     
            pahandle, [wave.reachBeep ; wave.reachBeep]);
        PsychPortAudio('Start', pahandle, 1, t0+exp.sound.seq_dur, 0);                         % on target beep
         display(sprintf('Movement started %4.2f s after fourth Beep, GOOD!', tmov-(t0+exp.sound.seqBeeps_time*4+fixt)))
...
if too_slow
      display(sprintf('%4.2f s from last beep, TOO SLOW',treach-(t0+exp.sound.seqBeeps_time*5+fixt)))
end 
% was it correct
